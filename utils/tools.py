...
def load_data_from_hdf5(file_path='data_buffer.h5', batch_size=1024):
    with h5py.File(file_path, 'r') as f:
        ...
        probs = f['mcts_probs'][indices]
        winners = f['winners'][indices]
...
def get_gpu_memory_usage_tegrastats():
    try:
        result = subprocess.run(['tegrastats'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=1)
        output = result.stdout.decode('utf-8')

        # 解析输出中的 GR3D 部分（包含 GPU 使用和显存）
        for line in output.split('\n'):
            if 'GR3D' in line:
                parts = line.split()
                for part in parts:
                    if 'FB' in part:  # FB 行通常包含显存使用情况
                        mem_info = part.split('/')[0]
                        return int(mem_info)  # 返回当前显存使用 MB
        return None
    except Exception as e:
        print(f"[ERROR] 获取显存失败: {e}")
        return None
