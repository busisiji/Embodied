...
# 添加全局变量用于跟踪窗口端口
_active_windows = {}  # 存储活跃窗口 {window_id: port}
_window_lock = threading.Lock()  # 用于线程安全
...
def create_window_visualization(host="127.0.0.1", port=8000):
    ...
    try:
        ...
        class ChessWindow:
            ...
            def start(self, window_id=None):
                """启动HTTP服务器"""
                self.window_id = window_id  # 保存窗口ID
                ...
                if window_id:
                    with _window_lock:
                        _active_windows[window_id] = self.port
            ...
            def stop(self):
                ...
                if self.server:
                    ...
                    if self.window_id:
                        with _window_lock:
                            _active_windows.pop(self.window_id, None)
...
class ChessWindowManager:
    """管理多个棋盘窗口的类"""

    def __init__(self, host="127.0.0.1", start_port=8000):
        self.host = host
        self.start_port = start_port
        self.windows = {}  # 存储窗口实例
        self.used_ports = set()  # 已使用的端口
    ...
    def create_window(self, window_id=None):
        """创建新的棋盘窗口"""
        port = self.get_available_port()
        self.used_ports.add(port)

        # 使用修改后的 create_window_visualization 创建窗口
        window_func = create_window_visualization(self.host, port)
        window = window_func()

        # 使用端口或指定ID作为键存储窗口
        key = window_id if window_id else port
        self.windows[key] = {
            'window': window,
            'port': port
        }
    ...
    def close_window(self, window_id):
        ...
        if window_id in self.windows:
            ...
            window_info['window'].stop()
            self.used_ports.discard(window_info['port'])
            del self.windows[window_id]

# 全局窗口管理器实例
window_manager = ChessWindowManager()

# 便捷函数
def create_new_chess_window(window_id=None):
    """创建新的棋盘窗口"""
    return window_manager.create_window(window_id)

def get_chess_window_by_id(window_id):
    """根据ID获取棋盘窗口"""
    return window_manager.get_window(window_id)
def get_active_window_ports():
    """获取所有活跃窗口的端口"""
    with _window_lock:
        return _active_windows.copy()

# 创建全局窗口获取函数
get_chess_window = create_window_visualization(host="0.0.0.0", port=8000)
