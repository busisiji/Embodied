...
class UserManager():
    """用户管理类"""
    def __init__(self):
        self.init_user("周文创","a123456789")
        self.init_user('admin','admin')
    def init_user(self, username, password):
        ...
        try:
            db.create_tables([User, ChessGameConfig], safe=True)

            # 检查是否已存在admin用户
            existing_user = User.get_or_none(User.name == username)
            if existing_user:
                self.create_user_tables(existing_user.user_id)
                print("  - 管理员账户已存在")
                return
            ...
            self.create_user_tables(user_id)


            print(f"  - 成功创建管理员账户 (用户名: {username}, 密码: {password})")
            return user_id
    ...
    def create_user_tables(self,user_id):
        """创建用户相关表"""
        ChessGameConfig.get_default_config(user_id)
